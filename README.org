...
Consider the following example, with two namespaces:

#+BEGIN_SRC nxml
  <exemplum xmlns="http://www.tei-c.org/ns/1.0">
    <egXML xml:id="vegest-listBibl" xmlns="http://www.tei-c.org/ns/Examples">
      <listBibl corresp="bibs.bib"/>
    </egXML>
  </exemplum>
#+END_SRC

There are two options to select the ~egXML~ element efficiently:

1) Write an XQuery (not XPath) that defines the namespaces, and then
   run the query:
   #+BEGIN_SRC xquery :exports code
       declare default element namespace "http://www.tei-c.org/ns/1.0";
       declare namespace eg="http://www.tei-c.org/ns/Examples";
       //eg:egXML
   #+END_SRC
   This should work for most (all?) implementations.
2) To select the elements directly from the prompt in Emacs, use
   [[https://www.w3.org/TR/xpath-31/#doc-xpath31-BracedURILiteral][BracedURILiteral]]s: ~M-x xquery-tool-query
   //Q{http://www.tei-c.org/ns/Examples}egXML~ For this, you will need
   a processor that conforms to XPath 3.0 or later.  For example,
   [[https://github.com/Saxonica/Saxon-HE/releases/download/SaxonHE12-5/SaxonHE12-5J.zip][SaxonHE12-5J]] will work once you configure where the ~.jar~ file is:
   ~M-x customize-variable RET xquery-tool-saxonb-jar~.
